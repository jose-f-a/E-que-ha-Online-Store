    ...
    async fazerCompra(req, res) {
        ...
        */
        ...
        await Promise.all(artigos.map(async(artigo) => {
            await connection.query("INSERT INTO linhacompra(compraid,produtoid,quantidade,preco) values(:compraid,:produtoid,:quantidade,:preco)RETURNING*;", {
                replacements: { compraid: compraid, produtoid: artigo.produtoid, quantidade: artigo.quantidade, preco: artigo.quantidade * artigo.preco }
            }).then(results => {
        ...
        console.log(total)
            ...
            */
        await connection.query("UPDATE compra set total=:total where compraid=:compraid RETURNING*;", {
