...
module.exports = {
  ...
  async getPendingCompras(req, res) {
    try {
      ...
      await connection
        .query(
          "SELECT * FROM compra, utilizador WHERE compra.userid = :userid AND compra.userid = utilizador.userid AND (compra.estadocompraid = 1 OR compra.estadocompraid = 2 OR compra.estadocompraid = 3) ORDER BY compraid DESC;",
          {
  ...
  async getFinishedCompras(req, res) {
    try {
      ...
      await connection
        .query(
          "SELECT * FROM compra, utilizador WHERE compra.estadocompraid = 4 AND compra.userid = :userid AND compra.userid = utilizador.userid ORDER BY compraid DESC",
          {
            replacements: { userid: userid },
          }
        )
